...
const GOAL_HELP = Object.freeze({
  scaffold: {
    ...
    safety: '需要 dynamic CAPTAIN/worker bootstrap 时，必须由 scaffold spec 显式携带 captain_canary_bootstrap/worker_canary_bootstrap；生成器会按 manifest decoder 验证并原样纳入 deterministic output。禁止生成后手改 manifest opt-in。',
  },
  'preclaim-issues': {
  ...
  'canary-bootstrap-plan': {
    usage: '<controller-root>/scripts/goal-control/canary-plan-launch.sh [--node-executable <canonical-absolute-path>] canary-bootstrap-plan --repository-worktree <frozen-goal-worktree> --manifest <json> --role CAPTAIN|DEV|REVIEW|RECEIPT --task <id> --expected-head <40sha> --operation-id <stable-id> --challenge <64hex> --canary-policy <repo-relative-path> --canary-policy-sha256 <sha256> [--json]',
    ...
    safety: 'CAPTAIN 与 worker 分别要求 captain_canary_bootstrap.protocol/worker_canary_bootstrap.protocol + exact committed policy marker；旧 worker-v1 marker 不授权 CAPTAIN，冻结旧 policy 不会被追认。本阶段没有 capability/GitHub/Browser/source/resource/environment authority；initial prompt 只允许目标 session 执行 canary-bootstrap-inspect 并返回实际 identity。',
  },
  ...
  'canary-bootstrap-prepare': {
    usage: '<controller-root>/scripts/goal-control/canary-plan-launch.sh [--node-executable <canonical-absolute-path>] canary-bootstrap-prepare --repository-worktree <frozen-goal-worktree> --manifest <json> --role CAPTAIN|DEV|REVIEW|RECEIPT --task <id> --expected-head <40sha> --operation-id <stable-id> --challenge <64hex> --canary-policy <repo-relative-path> --canary-policy-sha256 <sha256> --expected-identity-plan-sha256 <sha256> --expected-observation-sha256 <sha256> --worker-thread <id> --worker-host <id> --worker-worktree <actual-absolute-path> [--json]',
  ...
  'register-role': {
    usage: 'goalctl register-role --goal <id> --task <id> --role <role> --thread <id> [--host <id>] [--attempt <n>] [--lease-ms <ms>] [--status active|idle] [--launch-id <id>] [--event-id <stable-id>] [--bootstrap-capability-file <file>|--foreman-recovery-capability-file <file>|--authorizer-capability-file <file>|--actor-capability-file <existing-actor-file>] [--authorizer-thread <id>] [--worker-bootstrap-receipt <absolute-json> --worker-bootstrap-receipt-sha256 <sha256> --worker-bootstrap-operation-id <id> --worker-bootstrap-challenge <64hex> --worker-bootstrap-identity-plan-sha256 <sha256>] [--captain-bootstrap-receipt <absolute-json> --captain-bootstrap-receipt-sha256 <sha256> --captain-bootstrap-operation-id <id> --captain-bootstrap-challenge <64hex> --captain-bootstrap-identity-plan-sha256 <sha256>] [--json]',
    summary: 'Goal-wide FOREMAN 有三条明确路径：首次 bootstrap 登记；后续 task 以同一 Goal authority 的 identity/attempt/status/capability 做 projection；失联后走 recovery replacement，存在多个 current projections 时必须用 recover-expired-foreman 批量替换。CAPTAIN 与 worker 仍由当前授权链签发独立 capability；启用 worker canary bootstrap 的 Goal 会 seal actual worker identity，独立启用 captain canary bootstrap 的 Goal 会用五项 receipt binding seal CAPTAIN actual worktree identity。',
    safety: '这是持久化写命令：首次调用会发布 durable registration intent、actor capability 与 REGISTER_ROLE event。启用 worker canary bootstrap 时，DEV/REVIEW/RECEIPT 必须从 receipt-bound actual worker process cwd 调用，并逐项携带 canary 已验证的 receipt/hash/operation/challenge/plan；兄弟/CAPTAIN checkout 即使 HEAD 相同也拒绝且零控制面写入。启用独立 CAPTAIN bootstrap 时，CAPTAIN 同样必须从 receipt-bound actual cwd 携带 receipt/hash/operation/challenge/identity-plan 五项 binding；registration seal thread/host/cwd/gitdir/common-dir/branch/HEAD，START_P1 再重读 receipt 并复验 live identity。省略 --event-id 时使用绑定 task/host/thread/role/attempt 的 deterministic ID；响应丢失必须以同一 request、同一 ID、同五项 binding 和原 authorizer 或 actor capability exact retry。历史 exact retry 只返回原 registration/session/capability，不代表该历史 actor 仍具当前 operational authority；任何不同请求复用同一 ID 都拒绝。',
  },
...
function scaffoldGoal(cwd, options) {
  ...
  assertOnlyKeys(spec, ['schema_version', 'goal_id', 'title', 'mode', 'repository', 'base_head', 'protocol', 'preclaim', 'worker_canary_bootstrap', 'captain_canary_bootstrap', 'tasks'], 'INVALID_SCAFFOLD_SPEC', 'scaffold spec');
  ...
  const manifest = {
    ...
    ...(spec.captain_canary_bootstrap !== undefined
      ? { captain_canary_bootstrap: spec.captain_canary_bootstrap } : {}),
    tasks,
  };
